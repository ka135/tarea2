...
function imprimir(){
    ...
    function nuevoTitulo() {
        var contenido = document.getElementById('contenido'); // Obtener el div "contenido"
        var texto = document.getElementById("texto").value; // Obtener el texto del textarea
        contenido.innerHTML += '<h1>' + texto + '</h1>'; // Agregar el texto como título dentro de <h1>
    }
    function nuevoEnlace() {
        var contenido = document.getElementById('contenido'); // Obtener el div "contenido"
        var url = prompt("Ingrese la URL del enlace:"); // Pedir la URL al usuario
        var texto = prompt("Ingrese el texto del enlace:"); // Pedir el texto del enlace
    
        if (url && texto) { // Verificar que los valores no estén vacíos
            contenido.innerHTML += '<a href="' + url + '" target="_blank">' + texto + '</a><br>'; 
        } else {
            alert("Debe ingresar una URL y un texto para el enlace.");
        }
    }
